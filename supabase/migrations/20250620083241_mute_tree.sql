/*
  ...
  1. Changes
    - Set admin password to 'admin123' in auth.users table
    - Set admin password to 'admin123' in admins table
    - Ensure admin user exists in all required tables
    - Verify password synchronization works
...
BEGIN
  ...
  -- 3. Handle public.users table - first try to update by email, then by id
  -- Delete any existing records to avoid conflicts
  DELETE FROM public.users WHERE email = admin_email OR id = v_auth_user_id;
